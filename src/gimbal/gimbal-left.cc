...
namespace Gimbal
{
    ...
    void Gimbal_L::init(const std::shared_ptr<Robot::Robot_set> &robot) {
        ...
        Robot::hardware->register_callback<CAN0>(0x206, [&](const auto &frame) { yaw_motor.unpack(frame); });
        ...
        while (!inited) {
            ...
            yaw_motor.give_current = (int16_t)yaw_motor.pid_ctrler.out;
						...
						pitch_motor.give_current = (int16_t)-pitch_motor.pid_ctrler.out;
						 ...
						 //LOG_INFO("gyro %f %f %f\n", robot_set->gyro2_ins_pitch,robot_set->gyro2_ins_yaw,robot_set->gyro2_ins_roll);

            robot_set->gimbal2_yaw_set = robot_set->gyro3_ins_yaw;
            robot_set->gimbal2_yaw_offset = robot_set->gyro3_ins_yaw;
            if (fabs(robot_set->gimbal2_yaw_relative) < Config::GIMBAL_INIT_EXP &&
                ...
                init_stop_times = 0;
						...
						Robot::hardware->send<CAN0>(Hardware::get_frame(0x1FF, pitch_motor, yaw_motor));
        ...
        while (true) {
            ...
            if (robot_set->mode == Types::ROBOT_MODE::ROBOT_NO_FORCE) {
                ...
                delta1 += 0.008;
								...
								//LOG_INFO("gimbal2 yaw relative %f\n",robot_set->gimbal2_yaw_relative);
								if (robot_set->gimbal2_yaw_relative > -(M_PIf * 6 / 8) && !ret) {
									yaw_relative_pid.calc(robot_set->gimbal2_yaw_relative, -robot_set->gimbal1_yaw_relative - 0.1);
									pitch_absolute_pid.calc(robot_set->gyro2_ins_pitch, p);
								}else{
									yaw_relative_pid.calc(robot_set->gimbal2_yaw_relative, 0);
									pitch_absolute_pid.calc(robot_set->gyro2_ins_pitch, 0);
									ret = 1;
									if (fabs(robot_set->gimbal2_yaw_relative) < Config::GIMBAL_INIT_EXP){
										init_stop_times += 1;
									} else if (init_stop_times > Config::GIMBAL_INIT_STOP_TIME) {
										init_stop_times = 0;
										robot_set->set_mode(Types::ROBOT_IDLE);
									}
								}
                ...
                pitch_motor.pid_ctrler.calc(-pitch_gyro, pitch_motor.speed_set);
								...
								LOG_INFO("%f %f\n",yaw_abs,robot_set->gimbal2_yaw_set);
                yaw_absolute_pid.calc(yaw_abs, robot_set->gimbal2_yaw_set);
    ...
    void Gimbal_L::update_data() {
        ...
        yaw_gyro = -std::cos(robot_set->gimbal2_pitch_relative) * robot_set->gyro2_ins_yaw_v;
				yaw_abs = robot_set->gyro3_ins_yaw - robot_set->gimbal2_yaw_relative;
				robot_set->gimbal2_yaw_abs = yaw_abs;
